...
class D3D11Resource final : public ID3D12Resource {
   public:
    ...
    D3D12_RESOURCE_DESC* STDMETHODCALLTYPE GetDesc(D3D12_RESOURCE_DESC* desc) override;
    ...
    void TransitionTo(ID3D11DeviceContext* context, D3D12_RESOURCE_STATES newState);
    void UAVBarrier(ID3D11DeviceContext* context);
    void AliasingBarrier(ID3D11DeviceContext* context, D3D11Resource* pResourceAfter);
   ...
   private:
    ...
    static D3D11_USAGE GetD3D11Usage(const D3D12_HEAP_PROPERTIES* pHeapProperties);
