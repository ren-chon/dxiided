...
enum class DebugChannel : uint32_t { API = 0x01, SHADER = 0x02 };

enum class DebugLevel { NONE, ERR, WARN, FIXME, TRACE };
...
class Logger {
   public:
    ...
    bool HasChannel(DebugChannel channel) const {
        return (m_debugLevel & static_cast<uint32_t>(channel)) != 0;
    }
...
#else
#define TRACE(fmt, ...)                                               \
    dxiided::debug::Logger::Instance().Trace(__FILE__, __LINE__, fmt, \
                                             ##__VA_ARGS__)
#define ERR(fmt, ...)                                                         \
    dxiided::debug::Logger::Instance().Trace(__FILE__, __LINE__, "err: " fmt, \
                                             ##__VA_ARGS__)
#define WARN(fmt, ...)                                                         \
    dxiided::debug::Logger::Instance().Trace(__FILE__, __LINE__, "warn: " fmt, \
                                             ##__VA_ARGS__)
#define FIXME(fmt, ...)                                          \
    dxiided::debug::Logger::Instance().Trace(__FILE__, __LINE__, \
                                             "fixme: " fmt, ##__VA_ARGS__)
#endif
