...
const GUID IID_ID3D12Device1 = {
    0x77acce80,
    0x638e,
    0x4e65,
    {0x88, 0x95, 0xc1, 0xf2, 0x33, 0x86, 0x86, 0x3e}};
const GUID IID_ID3D12Device2 = {
    0x30baa41e,
    0xb15b,
    0x475c,
    ...
    switch (dxiiFeature) {
        case DXII_FEATURE_D3D12_OPTIONS: {
            ...
            if (FeatureSupportDataSize !=
                sizeof(DXII_FEATURE_DATA_D3D12_OPTIONS))
                return E_INVALIDARG;

            auto* data = static_cast<DXII_FEATURE_DATA_D3D12_OPTIONS*>(
                pFeatureSupportData);
        ...
        case DXII_FEATURE_SHADER_CACHE: {
            ...
            if (FeatureSupportDataSize !=
                sizeof(DXII_FEATURE_DATA_SHADER_CACHE))
                return E_INVALIDARG;

            auto* data = static_cast<DXII_FEATURE_DATA_SHADER_CACHE*>(
                pFeatureSupportData);
        ...
        case DXII_FEATURE_OPTIONS1: {
            ...
            auto* data =
                static_cast<DXII_FEATURE_DATA_OPTIONS1*>(pFeatureSupportData);
            data->WaveOps = TRUE;
        ...
        case DXII_FEATURE_SHADER_MODEL: {
            if (FeatureSupportDataSize !=
                sizeof(DXII_FEATURE_DATA_SHADER_MODEL))
                return E_INVALIDARG;

            auto* data = static_cast<DXII_FEATURE_DATA_SHADER_MODEL*>(
                pFeatureSupportData);
...
D3D11Device::CreateSampler(const D3D12_SAMPLER_DESC* pDesc,
                           D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor) {
    TRACE("(%p, %p)\n", pDesc, (void*)DestDescriptor.ptr);
}
...
void STDMETHODCALLTYPE D3D11Device::GetCopyableFootprints(
    ...
    TRACE("(%p, %u, %u, %llu, %p, %p, %p, %p)\n", pResourceDesc,
          FirstSubresource, NumSubresources, BaseOffset, pLayouts, pNumRows,
...
HRESULT STDMETHODCALLTYPE D3D11Device::CreatePipelineLibrary(
    ...
    TRACE(
        "D3D11Device::CreatePipelineLibrary called with blob %p, length %zu\n",
