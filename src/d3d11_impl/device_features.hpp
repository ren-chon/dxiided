...
struct DXII_FEATURE_DATA_D3D12_OPTIONS {
    BOOL DoublePrecisionFloatShaderOps;  // Support for double-precision float
                                         // operations in shaders
    BOOL OutputMergerLogicOp;  // Support for logic operations in output merger
    D3D12_SHADER_MIN_PRECISION_SUPPORT
        MinPrecisionSupport;  // Support for minimum precision in shaders
    D3D12_TILED_RESOURCES_TIER
        TiledResourcesTier;  // Level of support for tiled resources
    D3D12_RESOURCE_BINDING_TIER ResourceBindingTier;  // Resource binding tier
    BOOL PSSpecifiedStencilRefSupported;  // Support for pixel shader specified
                                          // stencil ref
    BOOL TypedUAVLoadAdditionalFormats;   // Support for additional formats in
                                          // typed UAV loads
    BOOL ROVsSupported;  // Support for Rasterizer Ordered Views (ROVs)
    D3D12_CONSERVATIVE_RASTERIZATION_TIER
        ConservativeRasterizationTier;  // Level of support for conservative
                                        // rasterization
    UINT64 MaxGPUVirtualAddressBitsPerResource;  // Maximum GPU virtual address
                                                 // bits per resource
    BOOL StandardSwizzle64KBSupported;  // Support for 64KB standard swizzle
    D3D12_CROSS_NODE_SHARING_TIER
        CrossNodeSharingTier;  // Level of support for cross-node sharing
    BOOL CrossAdapterRowMajorTextureSupported;  // Support for cross-adapter
                                                // row-major textures
    BOOL
        VPAndRTArrayIndexFromAnyShaderFeedingRasterizerSupportedWithoutGSEmulation;  // Support for VP and RT array index from any shader feeding rasterizer without GS emulation
...
struct DXII_FEATURE_DATA_OPTIONS1 {
    ...
    BOOL ExpandedComputeResourceStates;  // Support for expanded compute
                                         // resource states
