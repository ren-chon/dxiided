...
HRESULT WrappedD3D12ToD3D11CommandQueue::Create(WrappedD3D12ToD3D11Device* device,
                                  const D3D12_COMMAND_QUEUE_DESC* desc,
                                  REFIID riid, void** command_queue) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::Create %p, %p, %s, %p", device, desc,
    ...
    Microsoft::WRL::ComPtr<WrappedD3D12ToD3D11CommandQueue> queue =
        new WrappedD3D12ToD3D11CommandQueue(device, desc);

    return queue.CopyTo(reinterpret_cast<ID3D12CommandQueue**>(command_queue));
}

WrappedD3D12ToD3D11CommandQueue::WrappedD3D12ToD3D11CommandQueue(WrappedD3D12ToD3D11Device* device,
                                     const D3D12_COMMAND_QUEUE_DESC* desc)
    : m_device(device), m_desc(*desc) {
    TRACE(
        "WrappedD3D12ToD3D11CommandQueue::WrappedD3D12ToD3D11CommandQueue called %p, Type=%d, Priority=%d, "
        "Flags=%d",
...
// IUnknown methods
HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::QueryInterface(REFIID riid,
                                                            void** ppvObject) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::QueryInterface(%s, %p)",
...
ULONG STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::AddRef() {
    ULONG ref = InterlockedIncrement(&m_refCount);
    TRACE("WrappedD3D12ToD3D11CommandQueue::AddRef %p increasing refcount to %u.", this,
          ref);
    return ref;
}

ULONG STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::Release() {
    ULONG ref = InterlockedDecrement(&m_refCount);
    TRACE("WrappedD3D12ToD3D11CommandQueue::Release %p decreasing refcount to %u.", this,
          ref);
    if (ref == 0) {
        delete this;
    }
...
HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::GetPrivateData(REFGUID guid,
                                                            UINT* pDataSize,
                                                            void* pData) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::GetPrivateData(%s, %p, %p)",
...
HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::SetPrivateData(REFGUID guid,
                                                            UINT DataSize,
                                                            const void* pData) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::SetPrivateData %s, %u, %p",
          debugstr_guid(&guid).c_str(), DataSize, pData);
    return m_immediateContext->SetPrivateData(guid, DataSize, pData);
}

HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::SetPrivateDataInterface(
    REFGUID guid, const IUnknown* pData) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::SetPrivateDataInterface %s, %p",
          debugstr_guid(&guid).c_str(), pData);
    return m_immediateContext->SetPrivateDataInterface(guid, pData);
}

HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::SetName(LPCWSTR Name) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::SetName %ls", Name);
...
HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::GetDevice(REFIID riid,
                                                       void** ppvDevice) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::GetDevice %s, %p", debugstr_guid(&riid).c_str(),
          ppvDevice);
    return m_device->QueryInterface(riid, ppvDevice);
}

// ID3D12CommandQueue methods
void STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::UpdateTileMappings(
    ...
    TRACE(
        "WrappedD3D12ToD3D11CommandQueue::UpdateTileMappings %p, %u, %p, %p, %p, %u, %p, %p, "
        "%p, %d",
...
void STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::CopyTileMappings(
    ID3D12Resource* pDstResource,
    const D3D12_TILED_RESOURCE_COORDINATE* pDstRegionStartCoordinate,
    ID3D12Resource* pSrcResource,
    const D3D12_TILED_RESOURCE_COORDINATE* pSrcRegionStartCoordinate,
    const D3D12_TILE_REGION_SIZE* pRegionSize, D3D12_TILE_MAPPING_FLAGS Flags) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::CopyTileMappings %p, %p, %p, %p, %p, %d",
...
void STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::ExecuteCommandLists(
...
void STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::SetMarker(UINT Metadata,
                                                    const void* pData,
                                                    UINT Size) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::SetMarker %u, %p, %u", Metadata, pData, Size);
    // TODO: Implement debug markers
    FIXME("Debug markers not implemented.");
}

void STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::BeginEvent(UINT Metadata,
                                                     const void* pData,
                                                     UINT Size) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::BeginEvent %u, %p, %u", Metadata, pData, Size);
    // TODO: Implement debug events
    FIXME("Debug events not implemented.");
}

void STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::EndEvent() {
    TRACE("WrappedD3D12ToD3D11CommandQueue::EndEvent ");
    // TODO: Implement debug events
    FIXME("Debug events not implemented.");
}
...
HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::GetClockCalibration(
    UINT64* pGpuTimestamp, UINT64* pCpuTimestamp) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::GetClockCalibration %p, %p", pGpuTimestamp,
          pCpuTimestamp);
...
WrappedD3D12ToD3D11CommandQueue::GetDesc(D3D12_COMMAND_QUEUE_DESC* pDesc) {
    TRACE("WrappedD3D12ToD3D11CommandQueue::GetDesc(%p)", pDesc);
    if (pDesc) {
        *pDesc = m_desc;
    }
    return pDesc;
...
HRESULT STDMETHODCALLTYPE WrappedD3D12ToD3D11CommandQueue::create_swapchain(
    ...
    TRACE("WrappedD3D12ToD3D11CommandQueue::create_swapchain");

    return WrappedD3D12ToD3D11SwapChain::Create(m_device, factory, window, desc, 
